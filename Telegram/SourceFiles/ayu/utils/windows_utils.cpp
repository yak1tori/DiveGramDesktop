...
void processLegacy(const QString &iconPath) {
	...
	auto shortcut = appdata + "/Microsoft/Internet Explorer/Quick Launch/User Pinned/TaskBar/DiveGram Desktop.lnk";
	if (!QFile::exists(shortcut)) {
		shortcut = appdata + "/Microsoft/Internet Explorer/Quick Launch/User Pinned/TaskBar/DiveGram.lnk";
	}
	if (!QFile::exists(shortcut)) {
		return;
	}
...
void processNewShortcuts(const QString &iconPath) {
	...
	const auto shortcuts = {
		path + u"DiveGram Desktop/DiveGram.lnk"_q,
		path + u"DiveGram/DiveGram.lnk"_q,

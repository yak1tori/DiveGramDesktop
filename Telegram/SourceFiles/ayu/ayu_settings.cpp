...
void AyuSettings::load() {
	...
	try {
		...
		if (!p.contains("ghostModeSettings")) {
			...
			LOG(("DiveGramSettings: migrated ghost mode settings to per-account format"));
		}

		try {
			from_json(p, settings);
		} catch (...) {
			LOG(("DiveGramSettings: failed to parse settings file"));
		}
	} catch (...) {
		LOG(("DiveGramSettings: failed to read settings file (not json-like)"));
	}

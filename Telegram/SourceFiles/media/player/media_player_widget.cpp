...
, _lyricsButton(rightControls(), st::mediaPlayerLyricsButton)
...
, _speedController(
	...
	_lyricsButton->setClickedCallback([=] {
		LyricsWidget::Show(_controller);
	});
...
void Widget::updateControlsGeometry() {
	...
	if (_type == AudioMsgId::Type::Song) {
		...
		_lyricsButton->moveToRight(right, 0); right += _lyricsButton->width();
	}
...
void Widget::updateControlsVisibility() {
	...
	_lyricsButton->setVisible(_type == AudioMsgId::Type::Song);
...
void Widget::updateLyricsAvailability() {
	const auto current = instance()->current(AudioMsgId::Type::Song);
	const auto document = current.audio();
	const auto contextId = current.contextId();

	auto hasLyrics = false;
	const auto item = document
		? document->session().data().message(contextId)
		: nullptr;
	const auto original = item ? item->originalText().text : QString();
	if (!original.trimmed().isEmpty()) {
		hasLyrics = HasTimedLines(ParseLrc(original));
	}
	if (hasLyrics != _lastSongHasLyrics) {
		_lastSongHasLyrics = hasLyrics;
		_lyricsButton->setDisabled(!hasLyrics);
		_lyricsButton->setIconOverride(
			hasLyrics
				? nullptr
				: &st::mediaPlayerLyricsDisabledIcon,
			hasLyrics
				? nullptr
				: &st::mediaPlayerLyricsDisabledIconOver);
		updateControlsGeometry();
		updateControlsWrapGeometry();
	}
}

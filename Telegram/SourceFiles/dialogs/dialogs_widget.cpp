...
class LiquidGlassOverlay final : public Ui::RpWidget {
public:
	explicit LiquidGlassOverlay(not_null<Ui::InputField*> field)
	: Ui::RpWidget(field->parentWidget())
	, _timer([=] { update(); }) {
...
void LiquidGlassOverlay::paintEvent(QPaintEvent *e) {
	Painter p(this);
	const auto radius = st::dialogsFilter.borderRadius;
	Dialogs::LiquidGlass::PaintLiquidGlassEdge(
		p,
		rect(),
		radius);

	constexpr auto kSweepPeriod = crl::time(2600);
	const auto phase = (crl::now() % kSweepPeriod) / float64(kSweepPeriod);
	const auto rectf = QRectF(rect());
	const auto bandWidth = rectf.width() * 0.55;
	const auto bandLeft = rectf.left() - bandWidth
		+ phase * (rectf.width() + bandWidth * 2.);
	const auto band = QRectF(
		bandLeft,
		rectf.top(),
		bandWidth,
		rectf.height());
	auto light = QLinearGradient(band.topLeft(), band.topRight());
	light.setColorAt(0., QColor(255, 255, 255, 0));
	light.setColorAt(0.5, QColor(255, 255, 255, 26));
	light.setColorAt(1., QColor(255, 255, 255, 0));
	p.save();
	p.setRenderHint(QPainter::Antialiasing, true);
	p.setClipRect(rect());
	p.fillRect(band, light);
	p.restore();
}
...
, _singleMessageSearch(&controller->session()) {
	...
	_scroll->setCustomWheelProcess([=](not_null<QWheelEvent*> e) {
		...
		if (_wheelScrollAnimation.animating()) {
			_wheelScrollTo += delta;
		} else {
			_wheelScrollTo = _scroll->scrollTop() + delta;
		}
		const auto from = _scroll->scrollTop();
		const auto to = _wheelScrollTo;
	...
	_searchGlassOverlay = new LiquidGlassOverlay(_search);
	_searchGlassOverlay->setGeometry(_search->geometry());
...
void Widget::paintEvent(QPaintEvent *e) {
	...
	_glass.paint(p, r, size(), controller()->currentChatTheme());
	const auto bg = anim::brush(
		st::dialogsGlassDark,
		st::dialogsGlassDarkOver,
		_childListShown.current());
	...
	p.fillRect(
		style::rtlrect(width() - st::lineWidth, 0, st::lineWidth, height(), width()),
		st::shadowFg);
}

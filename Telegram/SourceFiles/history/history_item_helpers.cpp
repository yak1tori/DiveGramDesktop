...
TextWithEntities DropDisallowedCustomEmoji(
	...
	if (true) { // DiveGram: allow all premium emojis (via tg://emoji?id=...)
		return text;
	}
...
[[nodiscard]] TextWithEntities UnsupportedMessageText() {
	const auto siteLink = u"https://t.me/DiveGramReleases"_q;
	auto result = TextWithEntities{
		tr::lng_message_unsupported(tr::now, lt_link, siteLink).replace("Telegram", "DiveGram")
	};
...
HistoryMessageMarkupData UnsupportedMessageMarkup() {
	...
	row.emplace_back(
		Button::Type::Url,
		tr::lng_update_telegram(tr::now).replace("Telegram", "DiveGram"),
		Button::Visual(),
		QByteArray("https://t.me/DiveGramReleases"));

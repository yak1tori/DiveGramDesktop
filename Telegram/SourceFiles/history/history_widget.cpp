...
#include "dialogs/ui/liquid_glass.h"
...
constexpr auto kChatBarInsetV = 8;
constexpr auto kChatBarInsetH = 6;
constexpr auto kChatBarRadius = 18;

[[nodiscard]] QRect rectForChatBar(int viewportWidth, int top, int height) {
	return QRect(
		kChatBarInsetH,
		top,
		viewportWidth - 2 * kChatBarInsetH,
		height);
}
...
, _topShadow(this) {
	...
	_scroll->setCustomWheelProcess([=](not_null<QWheelEvent*> e) {
		...
		const auto delta = -Ui::ScrollDelta(e, false).y();
		if (!delta) {
			return true;
		}
		if (_wheelScrollAnimation.animating()) {
			_wheelScrollTarget += delta;
		} else {
			_wheelScrollTarget = _scroll->scrollTop() + delta;
		}
		const auto from = _scroll->scrollTop();
		const auto to = _wheelScrollTarget;
		_wheelScrollAnimation.stop();
		if (to != from) {
			_wheelScrollAnimation.start([=](float64 value) {
				const auto position = qRound(value);
				if (position != _scroll->scrollTop()) {
					_scroll->scrollToY(position);
				}
			}, from, double(to), st::slideDuration, anim::easeOutCubic);
		}
		return true;
	});
...
void HistoryWidget::refreshGiftToChannelShown() {
	...
	// DiveGram: hide gift button almost everywhere
...
void HistoryWidget::messagesReceived(
	...
	// Expects(_history != nullptr);
	if (!_history || !_peer) {
		return; // DiveGram: fix crash when using `saveDeletedMessages`
	}
...
// (_botStart|_unblock|_joinChannel|_muteUnmute|_reportMessages)
	...
	auto buttonsBottom = bottom - _attachToggle->height() - kChatBarInsetV;
	auto barBottom = bottom - kChatBarInsetV;
	auto left = st::historySendRight + kChatBarInsetH;
	if (_botMenu.button) {
	...
	const auto fieldTop = barBottom - fieldHeight() - st::historySendPadding;
	_field->moveToLeft(left, fieldTop);
	_richDraftPreview->moveToLeft(left, fieldTop);
	if (_fieldDisabled) {
		_fieldDisabled->moveToLeft(
			left,
			barBottom - fieldHeight() - st::historySendPadding);
	}
	auto right = st::historySendRight + kChatBarInsetH;
	_send->moveToRight(right, buttonsBottom); right += _send->width();
	_voiceRecordBar->moveToLeft(kChatBarInsetH, barBottom - _voiceRecordBar->height());
	...
	const auto fullWidthButtonRect = myrtlrect(
		kChatBarInsetH,
		barBottom - _botStart->height(),
		width() - 2 * kChatBarInsetH,
		_botStart->height());
...
void HistoryWidget::updateHistoryGeometry(
	...
	if (isChoosingTheme()) {
		...
		&& (isSearching()
			...
			|| isMuteUnmute()
			|| isReportMessages())) {
		newScrollHeight -= (_unblock->height() + kChatBarInsetV);
	} else {
...
int HistoryWidget::computeMaxFieldHeight() const {
	const auto available = height()
		...
		- ((_editMsgId
			...
			? st::historyReplyHeight
			: 0)
		- (2 * st::historySendPadding + kChatBarInsetV)
...
void HistoryWidget::drawField(Painter &p, const QRect &rect) {
	...
	const auto barRect = rectForChatBar(width(), backy, backh);
	if (!barRect.isEmpty()) {
...
void HistoryWidget::paintEvent(QPaintEvent *e) {
	...
	if (!controller()->currentChatTheme()->background().colorForFill) {
		auto frost = QPainter(this);
		frost.fillRect(e->rect(), QColor(255, 255, 255, 22));
	}

	Painter p(this);
	if (_discuss->isVisible()
		|| _muteUnmute->isVisible()
		|| _botStart->isVisible()
		|| _unblock->isVisible()
		|| _joinChannel->isVisible()
		|| _reportMessages->isVisible()) {
		const auto barHeight = _botStart->height();
		const auto barRect = QRect(
			kChatBarInsetH,
			height() - kChatBarInsetV - barHeight,
			width() - 2 * kChatBarInsetH,
			barHeight);
		Dialogs::LiquidGlass::PaintLiquidGlassPill(p, barRect, kChatBarRadius);
	}

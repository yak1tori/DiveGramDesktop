...
rpl::producer<TextWithEntities> Text() {
	return tr::lng_about_text2(
		...
		rpl::single(Ui::Text::Link(
			"GNU GPL",
			"https://github.com/DiveGram/DiveGramDesktop/blob/dev/LICENSE")),
		lt_github_link,
		rpl::single(Ui::Text::Link(
			"GitHub",
			"https://github.com/DiveGram/DiveGramDesktop")),
		tr::marked);
}
...
void AboutBox(not_null<Ui::GenericBox*> box, Window::SessionController* controller) {
	box->setTitle(rpl::single(u"DiveGram Desktop"_q));
	...
	box->addLeftButton(
		rpl::single(QString("@DiveGramReleases")),
		[box, controller]
		{
			box->closeBox();
